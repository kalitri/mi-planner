...
export default function Home() {
  ...
  const cargarTodo = async () => {
    try {
      ...
      // Convertir el array de menú a un objeto indexado por fecha
      const menuObj = {}
      m.data?.forEach(row => {
        menuObj[row.date] = {
  ...
  const NAV = [
    ...
    { id: 'quedadas', icon: '👥', label: 'Quedadas' },
      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <aside style={{ width: 200, background: G[600], display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          <div style={{ padding: '1rem', borderTop: `1px solid ${G[800]}50` }}>
            <div style={{ fontSize: 11, color: G[400], textAlign: 'center', marginBottom: 10 }}>
              {user.email}
            </div>
            <button onClick={handleLogout} style={{
              width: '100%', padding: '8px', border: `1px solid ${G[800]}60`,
        ...
        <main style={{ flex: 1, padding: '2rem', background: '#f0f7ea', overflowY: 'auto' }}>
          ...
          {view === 'agenda'   && <AgendaView   data={data} T={T} setModal={setModal} deleteEvento={deleteEvento} />}
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: G[400], fontSize: 14, marginTop: 4 }}>
          {todayTasks.filter(t => !t.done).length} tareas · {todayEvents.length + todayQ.length} eventos
        </p>
      </div>
      ...
      <SectionCard title="✅ Tareas de hoy">
        ...
        {todayTasks.map(t => (
          <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: `1px solid ${G[50]}` }}>
            <button onClick={() => toggleTask(t.id, t.done)} style={{
              width: 22, height: 22, borderRadius: '50%', flexShrink: 0,
              border: `2px solid ${t.done ? G[400] : G[200]}`,
              background: t.done ? G[400] : 'white',
              cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center'
            }}>
              {t.done && <span style={{ color: 'white', fontSize: 12, fontWeight: 700 }}>✓</span>}
            </button>
            <span style={{ flex: 1, fontSize: 14, color: t.done ? G[200] : G[800], textDecoration: t.done ? 'line-through' : 'none' }}>
              {t.text}
            </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {COMIDAS.map(c => (
            <div key={c} style={{ background: G[50], borderRadius: 8, padding: '10px 12px', border: `1px solid ${G[100]}` }}>
              ...
              <div style={{ fontSize: 14, color: todayMenu[c] ? G[800] : G[200], fontStyle: todayMenu[c] ? 'normal' : 'italic' }}>
                {todayMenu[c] || 'Sin planificar'}
              </div>
            </div>
          ))}
        </div>
      ...
      <SectionCard title="📅 Eventos del día">
        ...
        {[...todayEvents.map(e => ({...e, tipo: 'evento'})), ...todayQ.map(q => ({...q, tipo: 'quedada'}))].map(i => (
          <MiniEvento key={i.id} item={i} />
        ))}
      </SectionCard>
    </div>
  )
...
// VISTA: AGENDA
// ═══════════════════════════════════════════════════════════════════
function AgendaView({ data, T, setModal, deleteEvento }) {
  const all = [...data.eventos].sort((a,b) => a.date.localeCompare(b.date))
  const up   = all.filter(e => e.date >= T)
  const past = all.filter(e => e.date < T)

  return (
    <div style={{ maxWidth: 700 }}>
      <ViewHeader title="📅 Agenda" sub="Eventos y citas" onAdd={() => setModal({ type: 'evento' })} />
      {all.length === 0 && <EmptyState icon="📅" text="Sin eventos. ¡Añade el primero!" />}
...
function EventoCard({ e, past, onEdit, onDel }) {
  return (
    <div style={{ ...card, marginBottom: 10, opacity: past ? 0.6 : 1, borderLeft: `4px solid ${past ? G[200] : G[400]}`, borderRadius: '0 12px 12px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: G[800], marginBottom: 6 }}>{e.title}</div>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <Chip icon="📅">{fmtShort(e.date)}</Chip>
            {e.time  && <Chip icon="🕐">{e.time}</Chip>}
            {e.place && <Chip icon="📍">{e.place}</Chip>}
          </div>
          {e.notes && <div style={{ fontSize: 12, color: G[400], marginTop: 6, fontStyle: 'italic' }}>{e.notes}</div>}
        </div>
...
function QuedadaCard({ q, past, onEdit, onDel, onCycle }) {
  const ST = {
    ...
    confirmada: { bg: G[50],     color: G[600],     label: '✅ Confirmada' },
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
            ...
            <button onClick={onCycle} style={{ fontSize: 12, padding: '3px 10px', borderRadius: 20, border: 'none', cursor: 'pointer', background: s.bg, color: s.color, fontWeight: 500 }}>
              {s.label}
            </button>
          </div>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
...
function MenuView({ data, T, setMeal }) {
  ...
  const wLabel = wOff === 0 ? 'Esta semana' : wOff > 0 ? `+${wOff} semana${wOff > 1 ? 's' : ''}` : `${Math.abs(wOff)} sem. atrás`
      ...
      <div style={{ overflowX: 'auto', borderRadius: 12, border: `1px solid ${G[100]}` }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: 500 }}>
          ...
          <thead>
            <tr>
              ...
              {dates.map((d, i) => {
                ...
                return (
                  <th key={d} style={{ padding: '8px 4px', background: isT ? G[400] : G[600], color: isT ? G[900] : G[100], textAlign: 'center', border: `1px solid ${G[800]}30` }}>
          ...
          <tbody>
            {COMIDAS.map(meal => (
              <tr key={meal}>
                <td style={{ background: G[50], padding: '6px 10px', fontWeight: 600, color: G[400], fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.04em', border: `1px solid ${G[100]}` }}>
                  {meal}
                </td>
                {dates.map(d => {
                  const isT = d === T
                  const isEd = editing?.date === d && editing?.meal === meal
                  const val = data.menu[d]?.[meal]
                  return (
                    <td key={d} style={{ background: isT ? `${G[50]}99` : 'white', border: `1px solid ${G[100]}`, padding: 4, verticalAlign: 'top' }}>
                      {isEd ? (
                        <div>
                          <input value={editVal} onChange={e => setEditVal(e.target.value)}
                            onKeyDown={e => { if (e.key === 'Enter') commit(d, meal); if (e.key === 'Escape') setEditing(null) }}
                            style={{ width: '100%', fontSize: 12, padding: '3px 6px', border: `1px solid ${G[400]}`, borderRadius: 4 }}
                            autoFocus />
...
// ═══════════════════════════════════════════════════════════════════
// MODAL: FORMULARIO EVENTO / QUEDADA
// ═══════════════════════════════════════════════════════════════════
function FormModal({ modal, close, saveEvento, saveQuedada }) {
  const isEdit   = !!modal.edit
  const isEvento = modal.type === 'evento'
  const T = todayISO()

  const [form, setForm] = useState(
    modal.edit ? { ...modal.edit } : { title: '', date: T, time: '', place: '', notes: '', people: '', status: 'pendiente' }
  )
  ...
  const submit = async () => {
    ...
    if (isEvento) {
      ...
      const qPayload = { ...payload, people: form.people || null, status: form.status || 'pendiente' }
      if (form.id) qPayload.id = form.id
      await saveQuedada(qPayload)
    }
    close()
  }
  ...
  return (
    <div style={{ background: 'white', borderRadius: 16, padding: '1.5rem', width: 420, maxWidth: '95vw', border: `1px solid ${G[200]}`, maxHeight: '90vh', overflowY: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
        <h2 style={{ fontSize: 18, fontWeight: 600, color: G[800] }}>
          {isEdit ? 'Editar' : 'Nuevo'} {isEvento ? 'evento 📅' : 'quedada 👥'}
        </h2>
      ...
      <FF label="Título *">
        <input value={form.title} onChange={e => set('title', e.target.value)}
          placeholder={isEvento ? 'Ej: Cita médica' : 'Ej: Cena con Ana'}
          style={{ width: '100%' }} onKeyDown={e => e.key === 'Enter' && submit()} />
      </FF>
      ...
      <FF label="Lugar">
        <input value={form.place} onChange={e => set('place', e.target.value)} placeholder="¿Dónde?" style={{ width: '100%' }} />
      </FF>

      {!isEvento && <>
        <FF label="Con quién">
          <input value={form.people || ''} onChange={e => set('people', e.target.value)} placeholder="Ej: Ana, Carlos" style={{ width: '100%' }} />
        </FF>
        <FF label="Estado">
      ...
      <FF label="Notas">
        <textarea value={form.notes || ''} onChange={e => set('notes', e.target.value)}
          placeholder="Notas opcionales..." style={{ width: '100%', minHeight: 70 }} />
      </FF>
...
function MiniEvento({ item }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0', borderBottom: `1px solid ${G[50]}` }}>
      <div style={{ width: 10, height: 10, borderRadius: '50%', background: item.tipo === 'quedada' ? G[200] : G[400], flexShrink: 0 }} />
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 500, color: G[800] }}>{item.title}</div>
        {item.place && <div style={{ fontSize: 12, color: G[400] }}>📍 {item.place}</div>}
      </div>
      {item.time && <div style={{ fontSize: 13, color: G[600] }}>{item.time}</div>}
    </div>
  )
}
